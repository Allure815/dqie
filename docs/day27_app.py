...
def detect_anomaly(df):
    ...
    if latest > 2 * avg:
        return f"⚠️ Anomaly: Spike ({round(latest,2)} sec)"
    return None
...
def calculate_risk(df):
    ...
    # Duration factor
    if latest["duration_seconds"] > 30:
        score += 40
    elif latest["duration_seconds"] > 10:
        score += 20
    else:
        score += 5
    ...
    # Anomaly factor
    avg = df["duration_seconds"].mean()
    if latest["duration_seconds"] > 2 * avg:
        score += 30
...
def get_recommendation(score, trend):
    ...
    if trend and "Increasing" in trend:
        return "⚠️ Performance degrading, monitor closely"

    if score >= 50:
        return "🔧 Optimize queries / check indexes"
...
st.title("🧠 DQIE - Decision Intelligence Engine")
...
while True:
    ...
    if not alerts_df.empty:
        ...
        col2.metric("Avg Duration", round(alerts_df["duration_seconds"].mean(), 2))
        col3.metric("Max Duration", round(alerts_df["duration_seconds"].max(), 2))
    ...
    score = calculate_risk(alerts_df)
    priority = get_priority(score)
    recommendation = get_recommendation(score, trend)
    health = system_health(alerts_df)
